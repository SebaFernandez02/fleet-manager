...
public class PostgresControlRepository implements ControlRepository, RowMapper<Control> {
    ...
    private void update(Control control) {
        try {
            var sql = "update controls set status = ?, assigned = ? where id = CAST(? as UUID)";

            this.jdbcTemplate.update(sql, control.status().name(), control.assigned().value(), control.id().value());
    ...
    private void create(Control control) {
        try{
            var sql= """
                    insert into controls (id, type, subject, description, id_vehicle, priority, date, status, assigned) values 
                                         (CAST(? as UUID), ?, ?, ?, ?, ?, ?, ?, ?)
                    """;
...
//    }
    ...
    public Control mapRow(ResultSet rs, int rowNum) throws SQLException {
        try{
            ...
            var idVehicle = rs.getString("id_vehicle");
            var priority = rs.getString("priority");
            var date = rs.getTimestamp("date").toLocalDateTime();
            var status = rs.getString("status");
            var assigned = rs.getString("assigned");

            return Control.build(id, type, subject, description, idVehicle, priority, date, status, assigned);
